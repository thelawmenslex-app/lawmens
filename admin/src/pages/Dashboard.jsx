...
const Dashboard = () => {
  ...
  const stats = [
    ...
    { name: 'Active Users (7 Days)', value: data?.activeUsers7Days || 0, icon: UserCheck, color: 'bg-emerald-500' },
    { name: 'Premium Subscribers', value: data?.premiumUsers || 0, icon: DollarSign, color: 'bg-amber-500' },
    { name: 'Trial Accounts', value: data?.trialUsers || 0, icon: Activity, color: 'bg-purple-500' },
    { name: 'Total Law Books', value: data?.totalBooks || 0, icon: Library, color: 'bg-cyan-500' },
    { name: 'Total Sections/Laws', value: data?.totalLaws || 0, icon: FileCode, color: 'bg-indigo-500' },
  ];

  return (
    <div className="space-y-8">
      {/* Dynamic welcome and system banner */}
                ...
                <div className="flex items-center gap-4 flex-1 justify-end ml-4">
                  <div className="h-2 w-32 bg-slate-100 rounded-full overflow-hidden">
                    <div 
                      ...
                      style={{ width: `${Math.min(100, (prof.count / (data.totalUsers || 1)) * 100)}%` }}
                    ></div>
                  </div>
                  <span className="text-sm font-bold text-slate-800 w-8 text-right">{prof.count}</span>
                </div>
              </div>
            ))}
