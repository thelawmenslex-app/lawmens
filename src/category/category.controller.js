...
const getEndUseById = async (req, res) => {
    try {
        const { categoryId } = req.params;
        const category = await categoryService.getCategory({ _id: categoryId });
        return sendResponse(res, true, 200, 'Category details retrieved.', category);
    } catch (error) {
        errorHandler(error, res);
    }
};

module.exports = { addEndUse, getEndUses, getEndUseById };
